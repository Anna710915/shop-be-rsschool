...
import { importFileParserHandler } from '../../../lib/lambda/importFileParser';
...
describe('importFileParserHandler', () => {
  ...
  it('should process CSV file, copy it to parsed folder, and delete the original', async () => {
    ...
    await importFileParserHandler(s3Event);
  ...
  it('should log an error if an exception occurs', async () => {
    ...
    await importFileParserHandler(s3Event);
    expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Error handling S3 event:'), expect.any(Error));
  });
});
